...
What's still open — the "GrayScale" product family (Apexus quote tool, Tempus scheduling, Solus branding, suggested-moves intelligence), a client onboarding journey (automated portal invite + walkthrough wizard), and client portal template variations (service vs. subscription-only clients) — is tracked in `docs/Open-Work-Brief.md`, along with what's blocked and on what.
