...
- **Rollback:** depends on which deploy path is active.
  - **Right now (local-source deploy, no GitHub connection yet):** `git checkout <previous-good-commit>` then `firebase deploy --only apphosting` — this builds and rolls out that older commit as a brand-new rollout. There's no "revert to previous build" shortcut in this mode; redeploying old source is the rollback.
  - **Once GitHub Developer Connect is connected:** `firebase apphosting:rollouts:create grayportal --git-commit <previous-good-sha> --force` rolls straight back to any prior commit without touching the local checkout — this is the better path once available, since it doesn't depend on what's checked out locally.
  - Neither path has been rehearsed as an actual incident drill yet — worth doing once, deliberately, rather than trusting it untested.
