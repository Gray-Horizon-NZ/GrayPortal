...
### 1.1 Apexus — live quote tool — **built 2026-09-06, wired live same day**

Settled differently than either option §7 item 2 posed: Max already had a finished standalone tool (`templates/Proposal and Quote/GrayHorizon_Quote_Builder.html` — a self-contained HTML/CSS/JS quote builder with package presets, discount logic, and a "Create Document" export that bundles a curated menu into a client-facing standalone quote page) — not a calculator to build from scratch, and not an external tool to integrate via API. Copied to `public/apexus/quote-builder.html`, given its own top-level nav tab at the bottom of the Finance group (`/apexus`, `src/app/(app)/apexus/page.tsx`, `src/app/(app)/layout.tsx`'s `navItems`) rather than living at the foot of `/pricing` as first built — embedded via `<iframe>` there, with an "open full screen" link.

**No longer running on hardcoded pricing** — same-day follow-up, per Max: the tool's own `GS_ITEMS`/`GA_ITEMS`/`CREATIVE_ITEMS`/`WEBDEV_TIERS`/`AO_ITEMS`/`PART2_ITEMS` data blocks are gone. A new admin-gated route, `GET /api/apexus-pricing` (`src/app/api/apexus-pricing/route.ts`), reads live off `dal/pricing.ts`'s `serviceModules`/`serviceItems` (same table `/pricing` renders from — "Current" price, matching that page's own "Current is the default rate to quote" convention) and off `grayscale_products` for the Gray Scale section. The tool's own script now fetches this on load (same-origin, so the admin's session cookie travels automatically since it's only ever loaded inside GrayPortal's own iframe) and builds its catalogue UI from the response — see `loadCatalogueAndRender()`/`renderCatalogueUI()` near the top and bottom of the file's `<script>` block. Item ids throughout (the TDI/Ads dependency check, the three quick-start `PACKAGES` presets) were repointed from the tool's old short codes (`tdi`, `gam`, `mam`, …) to the live catalogue's real ids (`gs-tracking-infra`, `ga-google-ads-mgmt`, `ga-meta-ads-mgmt`, …). SS (Systems & Software) stays manual-entry only, unchanged — no fixed catalogue price exists for it (Yuvi-multiplier, value-adjusted, per the framework doc itself). Editing a price on `/pricing` or `/grayscale-products` now flows through on Apexus's next load, no separate file to maintain.

The `/grayscale-products` admin page now links directly to `/apexus` ("Build a quote →"), satisfying the "link the GrayScale tab to the quote calculator" ask — this is the one direction it goes (admin tool → admin tool); nothing was added on the client-portal side, since Apexus is an internal proposal-drafting tool, not client-facing.

**Also same day — GrayScale pricing.** `grayscale_products` gained a real, admin-editable `monthlyPriceNzd` column (migration `db/migrations/0032_legal_ben_grimm.sql` — **needs applying by hand in Neon's SQL editor**, same handoff as every migration in this repo) with a form field on `/grayscale-products`. Initial values were sourced verbatim from the live `grayhorizon.nz/grayscale` marketing page's own JSON-LD product catalogue (Osseus $250, Fidelis $340, Apexus $220, Aurum $240, Meridian $195, Stratum $420, Memoria $380, Tempus $420, Solus $190 — all $/mo NZD) via a new one-off script, `scripts/backfill-grayscale-pricing.mjs` — **needs running by hand once the migration lands** (same DATABASE_URL_UNPOOLED-direct pattern as `import-grayscale-products.mjs`; this session's own tool-use classifier still blocks running it directly against prod, per §9.4). Apexus's Gray Scale section now prepopulates from this live table (still fully editable/removable per-row, same as before) instead of starting blank.

**Cosmetic experiment, same day, not yet confirmed:** the tool's `--black` CSS variable (drives most of its text and button-chrome) was swapped from near-black (`#1c1c1c`) to GrayPortal's own gold accent (`--gh-accent`, `#c2a468`), per Max's "make the text gold, see how it looks" — a one-line, easily-reverted change (comment left in the CSS pointing at the original value). Needs Max's eyes on the actual rendered tool before treating this as a real decision either way.
...
**Status:** Built, 2026-09-06 — dropped from open-work tracking (Max: not worth carrying further). See §10 for what shipped. Foundation (token/invite mechanism), the wizard shell (7 steps, §10.3), §4.5's document-attach mechanism, and §4.6's admin checklist are all built. Step 6 (GrayScale) stays a static "ask us" note by design, not a placeholder awaiting a backend — accepted as final, not reopened when GrayScale (§1) eventually gets scoped unless Max asks for it specifically.
...
(Tempus and Suggested moves above are also in this deferred category, but tracked with their own numbered sections since they're part of the named GrayScale family. Tempus is next in line — see §13.)

**Dropped from tracking, 2026-09-06** (Max: not worth carrying as open work) — general workflow/automation engine, agreement/contract e-signature generation, Agent Inbox, and real performance-metrics sourcing for "Performance snapshot" (the Google Ads/GA4 API integration explored 2026-09-05 — manual entry into `client_metrics_snapshots` stays as-is, permanently, not just for now).
...
8. ~~Existing prospect contacts and consent — do current pipeline contacts have a sufficient existing-relationship basis to receive a first blast under §2.3, or should the first prospect send be preceded by some explicit opt-in step?~~ **Resolved 2026-09-06: no opt-in step.** Max's framing: these are personalised relationships, not a cold/purchased list, so the existing-relationship basis in §2.3 is sufficient as designed — no `marketingOptOut`-style consent gate needed beyond what's already scoped there.
...
- ~~**Still not built:** real GrayScale product content (step 6 stays a static "ask us" note until §1's product family gets scoped).~~ **Dropped, 2026-09-06** — step 6 stays a static note permanently, not a placeholder; not reopened by §1 landing.
...
- **Task funnel tagging**: `tasks.funnel_stage` (nullable enum: next/doing/done, independent of `tasks.status`) — an optional "roadmap stage" field added to the existing edit popup (`EditTaskButton.tsx`), shown only when a task has a clientId. Tags tasks Max is already managing in Master Task View/the real portal-preview-turned-live task list; no second list to maintain.
- **Roadmap redesigned** as "Focus Dial + Now/Next/Later" (`gray-portal-roadmap-combined.html`) — `roadmap_items` repurposed as ordered *phases* (title/description/status feed the dial's headline/narrative/current-phase state) instead of flat milestones; `DeliveryTab.tsx` gained an edit form per phase (previously create+delete only). The Now/Next/Later columns are fed by funnel-tagged tasks, not a second manually-maintained list. One shared component, `src/components/portal/RoadmapWidget.tsx` (full on `/portal/work`, compact on the dashboard), replaces the old flat roadmap list and the `MilestonesTimeline` SVG chart (deleted) in both the real portal and the admin preview.
- **Portal preview is now genuinely live**, not a hand-maintained mirror. Clicking "View client portal" (`startPortalPreviewAction`, `clients/actions.ts`) sets a short-lived httpOnly cookie (`gh_admin_preview_client`) and sends the admin into the real `/portal` route tree. `withCaller` (`src/lib/dal/auth.ts`) resolves that cookie into the admin's effective `Caller.clientId`/`isAdminPreview` for the request, `requireClientScope` (`src/lib/dal/session.ts`) returns that instead of throwing for an admin (a real client's own path is untouched — same function, same guarantee). The two genuinely client-only mutations (referral submit, GrayScale request) use a new stricter `requireRealClientScope` that never honors the preview cookie, so an admin can look but never submit as the client; task management stays the one interactive capability in preview, exactly as the old mirror's own stated design. `PortalPreviewShell.tsx`, its `portal-preview/` route, `TaskListPreview.tsx`, and `getClientPortalPreviewData` are deleted — nothing hand-duplicates the real pages anymore. A persistent "Previewing as {client} · Exit preview" banner shows only in this state.

**Deploy notes:**
- `db/migrations/0030_spotty_xorn.sql` (`grayscale_products`), `db/sql/029_grayscale_products.sql`, and `scripts/import-grayscale-products.mjs` were all run by hand mid-session and confirmed live (9 products, RLS enabled) — no longer outstanding.
- **One new migration needs applying by hand**: `db/migrations/0031_overconfident_thor.sql` (`CREATE TYPE task_funnel_stage`, `ALTER TABLE tasks ADD COLUMN funnel_stage`). No accompanying `db/sql` file needed — `tasks` already has RLS/grants covering every column. Until this runs, the new "Roadmap stage" field on tasks will error.
...
- This session's work was pushed across several commits as it was reviewed/approved in rounds rather than one large one at the end — see git log for the exact split.

---

## 13. Session handoff — 2026-09-06

Picked up as a documentation-pruning pass (this file had accumulated deferred items Max no longer wants tracked) plus one real bug fix.

**Doc changes:**
- §6 (Deferred, not scheduled): removed the general workflow/automation engine, agreement/contract e-signature generation, Agent Inbox, and real performance-metrics sourcing entries — Max doesn't want these carried as open work at all, not even as "deferred." AI Calendar Assistant / AI Docs Assistant is the only item still sitting in that bucket.
- §4 (Client onboarding journey): dropped from open-work tracking entirely. Step 6 (GrayScale discount) staying a static "ask us" note is now the accepted final state, not a placeholder waiting on §1 — it will not be reopened just because GrayScale eventually gets scoped.
- §7 item 1 resolved: "GrayScale" is confirmed as the correct umbrella term.
- §7 item 4 (Tempus/Calendar-sync): not resolved — Max didn't recognize the question as posed. Reframed here: today's Calendar sync is one-way (reads only) by a prior deliberate decision; a real Tempus booking tool needs to *write* events, so that one-way decision needs revisiting whenever Tempus is actually built. Not urgent to answer today, but live again now that a scheduling feature is next up (see below).
- §7 item 8 resolved: no opt-in step for prospect email blasts. Max's reasoning — existing pipeline contacts are personalised relationships, not a cold/purchased list — so §2.3's existing-relationship basis is sufficient as designed.

**Bug fixed** (§9.3 item 1): a task auto-created by a `STAGE_TASK_RULES` rule kept showing on the task list forever after its linked deal moved to Lost/Dormant or was soft-deleted — `listAllTasks()` (`src/lib/dal/tasks.ts`) had no awareness of the deal's own stage/deletion at all. Fixed by excluding a deal-linked task from the query when its deal is dead (`stage` in `Lost`/`Dormant`, or the deal row itself soft-deleted) **unless** the deal's company has since become a real client (the existing `dealClients` join already handles that fold, so it's left untouched) — a task with no `dealId` is never affected. `Won` is deliberately not treated as dead here, since a won deal's company routinely becomes a client and that path already resolves correctly. Verified with `tsc --noEmit` (clean aside from one pre-existing, unrelated failure in `tests/dal/requireClientScope.test.ts`); no live browser test this session (see §10.2's note on why — same NTFS/Turbopack constraint, unchanged).

**Revised same session:** the scheduling feature (Tempus) flagged above as next up is **not** happening right now — Max called it not essential currently. Left deferred per §1.3/§6, no change in status; §7 item 4 stays open and unpressing.

**Built instead, same session — §1.1 Apexus (see §1.1 for full detail):** Max already had a finished standalone quote-builder tool (`templates/Proposal and Quote/GrayHorizon_Quote_Builder.html`) sitting outside the app. Copied to `public/apexus/quote-builder.html`. §7 item 2 settled by this, though not in either way that question originally posed it — see §1.1 and item 2 for the distinction from the still-unscoped $220/mo branded GrayScale "Apexus" product. `README.md` updated to reflect this (moved out of the "still open" list, added to "What's built").

**Follow-up, same session, per Max's next round of feedback (full detail in §1.1):** what shipped above wasn't quite what Max wanted — three changes, all same day:
1. Wired live: the tool no longer carries its own hardcoded pricing block. A new `GET /api/apexus-pricing` route feeds it the real `service_items`/`service_modules` catalogue and the `grayscale_products` table on every load.
2. `grayscale_products` gained a real, admin-editable monthly price field, seeded from the live grayhorizon.nz/grayscale marketing page's own catalogue — Apexus's Gray Scale section now prepopulates from it.
3. Moved out from the foot of `/pricing` into its own nav tab (`/apexus`, bottom of the Finance group), with a "Build a quote →" link added from `/grayscale-products`.
